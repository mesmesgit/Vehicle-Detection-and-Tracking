...
def convert_cxcy_to_cv2bbox(bbox, img_dim=(1280, 720)):
    """
    Helper function to convert bounding boxes
    bbox = [cx, cy, width, height]
    bbox_cv2 = [left, top, right, bottom]
    """
    cx, cy, width, height = bbox
    left = int(np.maximum(0, cx - (width/2.0)))
    top = int(np.maximum(0, cy - (height/2.0)))
    right = int(np.minimum(img_dim[0], cx + (width/2.0)))
    bottom = int(np.minimum(img_dim[1], cy + (height/2.0)))
    return (left, top, right, bottom)


def draw_box_label(img, bbox, box_color=(0, 255, 255), show_label=True):
    '''
    ...
    font_size = 0.7
    font_color = (0, 0, 0)
    left, top, right, bottom = convert_cxcy_to_cv2bbox(bbox)
